...
def load_dataset(data_dir, img_size):
    ...
    for img in img_files:
        try:
            ...
            coords_x = x/img_size
            coords_y = y/img_size
            coords = [(q, r) for q in range(int(coords_x))
                      for r in range(int(coords_y))]
            for coord in coords:
...
def get_test_set(original_size, shrunk_size):
    ...
    x = [scipy.misc.imresize(get_image(q, original_size),
                             (shrunk_size, shrunk_size)) for q in imgs]
...
def get_batch(batch_size, original_size, shrunk_size):
    ...
    window = [x for x in range(
        int(counter*batch_size), int((counter+1)*batch_size))]
